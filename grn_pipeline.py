...
.nw-side{width:240px;display:flex;flex-direction:column;gap:1rem}
...
<div style="overflow-x:auto"><table id="hubTable"><thead><tr><th>Rank</th><th>Gene</th><th>Locus</th><th>Betweenness</th><th>Degree</th><th>COG</th><th>Community</th><th>logFC</th><th>FDR</th><th>Essential</th></tr></thead><tbody id="hubBody"></tbody></table></div></div>
<div class="card" style="margin-bottom:1.5rem"><h3>Perturbed Pathways (DE-enriched communities)</h3>
<div style="overflow-x:auto"><table id="pathTable"><thead><tr><th>Community</th><th>Size</th><th>DE Count</th><th>Enrichment</th><th>padj</th><th>Dominant COG</th></tr></thead><tbody id="pathBody"></tbody></table></div></div>
...
function init(){
  ...
  renderTable();renderHubs();renderPathways();drawCharts();initNetwork();
}
...
function showTab(t){
  ...
  if(t==='network')setTimeout(()=>{resizeCanvas();drawNetwork();},50);
}
...
function renderHubs(){
  ...
  document.getElementById('hubBody').innerHTML=nodes.map(([locus,a],i)=>
    `<tr><td>${i+1}</td><td><b>${a.gene_name}</b></td><td>${locus}</td>
    <td>${a.betweenness.toFixed(4)}</td><td>${a.degree.toFixed(3)}</td>
    <td>${a.cog_category}</td><td>${a.community}</td>
    <td>${a.logFC!==null?a.logFC.toFixed(2):'-'}</td><td>${a.FDR!==null?a.FDR.toExponential(1):'-'}</td>
    <td>${a.is_essential?'✓':''}</td></tr>`
  ).join('');
}

// ── PATHWAYS ──
function renderPathways(){
  // Compute DE enrichment per community client-side
  const commStats={};
  const allDE=Object.values(NODE_ATTRIBUTES).filter(n=>n.is_de).length;
  const totalNodes=Object.keys(NODE_ATTRIBUTES).length;
  Object.entries(NODE_ATTRIBUTES).forEach(([locus,a])=>{
    const c=a.community;if(c<0)return;
    if(!commStats[c])commStats[c]={size:0,de:0,cog:{},domCog:'-'};
    commStats[c].size++;
    if(a.is_de)commStats[c].de++;
    const cog=a.cog_category;
    commStats[c].cog[cog]=(commStats[c].cog[cog]||0)+1;
  });
  Object.values(commStats).forEach(s=>{
    const sorted=Object.entries(s.cog).filter(([k])=>k!=='-').sort((a,b)=>b[1]-a[1]);
    if(sorted.length)s.domCog=sorted[0][0];
    s.enrichment=(s.de/s.size)/(allDE/totalNodes);
  });
  const rows=Object.entries(commStats).sort((a,b)=>b[1].de-a[1].de).filter(([,s])=>s.de>0);
  document.getElementById('pathBody').innerHTML=rows.map(([c,s])=>
    `<tr><td>Community ${c}</td><td>${s.size}</td><td>${s.de}</td>
    <td>${s.enrichment.toFixed(2)}x</td><td>-</td><td>${s.domCog}</td></tr>`
  ).join('');
}
...
function initNetwork(){
  ...
  // Init positions
  nwNodes.forEach((n,i)=>{
    nwPos[n.id]={x:canvas.width/2+Math.cos(i*0.5)*200,y:canvas.height/2+Math.sin(i*0.5)*200,vx:0,vy:0};
  });
  // Interactions
...
function resizeCanvas(){
  ...
  c.width=c.offsetWidth||820;c.height=600;
}
...
function resetLayout(){scale=1;offsetX=0;offsetY=0;const c=document.getElementById('nwCanvas');
  nwNodes.forEach((n,i)=>{nwPos[n.id]={x:c.width/2+Math.cos(i*0.5)*200,y:c.height/2+Math.sin(i*0.5)*200,vx:0,vy:0};});
  drawNetwork();}
...
function drawNetwork(){
  ...
  const ec={negative:'var(--neg)',positive:'var(--pos)',suppression:'var(--sup)',PPI:'var(--ppi)'};
